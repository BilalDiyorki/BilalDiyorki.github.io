document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvas) { /* ... (V13-V18 arası aynı parçacık kodu) ... */ 
        ...
        function drawLink(p1, p2, opacity, isMouse = false) { if (!p2 || p2.x == null || p2.y == null) return; ctx.beginPath(); ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y); ctx.strokeStyle = isMouse ? `rgba(48, 185, 227, ${opacity * 0.3})` : `rgba(48, 185, 227, 0.1)`; ctx.lineWidth = isMouse ? 1.5 : 1; ctx.stroke(); }
